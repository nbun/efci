...
data Thunking v :: Type -> (Type -> Type) -> Type where
   Thunk :: [ScpVarIndex] -> Thunking v Ptr' (OneSub v)
   ...
   con (S (Enter op)) = MC $ \th -> con $ S $ Enter $ fmap (go th) op
     where
      go th hhx = do
         (th', hx) <- unMC hhx th
         return (unMC hx th')
   con (L (Node (Inl3 (Thunk vars)) l st k)) = MC $ \(TS fresh rm im) -> ctrace ("thunked " ++ show fresh) $ do
      let ptrs = map (\v -> fromJust $ Map.lookup v rm) vars
      unMC (k ((fresh) <$ l)) (TS (fresh + 1) rm (Map.insert (fresh) (ptrs, Left (unsafeCoerce $ st One))  im))
      ...
      in case Map.lookup ptr th of
         ...
         Just e -> case e of
            (live, Left t) -> do
             ...
             strace (show lv ++ " " ++ show (vars lv)) (return ())
             unMC (k lv) (ctrace ("evaluate " ++ show ptr ++ show lv) (TS fresh' rm' (Map.insert ptr (vars lv, Right lv) th')))
            (_, Right lv) -> ctrace ("memoized " ++ show ptr ++ show lv) $ unMC (k lv) ts
   con (L (Node (Inl3 (RunGC vs)) l _ k)) = MC $ \ts@(TS _ rm _) -> 
...
data ThunkStore l v = forall m. TS Int (Map.Map ScpVarIndex Ptr') (Map.Map Ptr' ([Ptr'], Either (l () -> MC m l v (l v)) (l v)))
...
showTS (TS i _ m) = show i ++ " \n"
  ++ concatMap ((++ "\n") . show . (\(i, (p,(Right lv))) -> (i, (p, lv)))) ((filter (\(_, (_,e)) -> isRight e)) (Map.toList m))
  ++ concatMap ((++ "\n") . show . (\(i, (p,(Left lv))) -> (i, (p, "-")))) ((filter (\(_, (_,e)) -> isLeft e)) (Map.toList m))
  ++ "\n evaluated: " ++ show (length (filter (\(_,e) -> isRight e) $ map snd (Map.toList m)))
  ++ " thunks: " ++ show (length ((filter (\(_,e) -> isLeft e)) $ map snd (Map.toList m)))
{-# INLINE showTS #-}
...
garbageCollector ptrs ts@(TS i rm im) = TS i rm (Map.filterWithKey (\k _ -> k `elem` allPtrs) im)
  where
...
pointsTo :: ThunkStore l v -> Ptr' -> [Ptr']
pointsTo (TS _ _ im) p = case Map.lookup p im of
   Nothing -> []
   Just (ps, _) -> ps